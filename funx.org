...
#+CAPTION: Strategies in CASNET glaucoma consultation program, developed at Rutgers (1971-8) based on /causal association networks/.
#+attr_texinfo: :width 200px :center t
[[./CASNET.jpg]]
...
** NXP Archeology and vestigial software artifacts
...
#+BEGIN_QUOTE
  ...
  - Smart's "topic neutral" analyses led Armstrong and Lewis to a functionalist analysis of mental concepts. (In which /causality/ is emphasized[fn:13].)
...
- environment :: used to hold bindings of variables to values during computation.
...
The control list constitutes the program of the abstract machine and tells it how to move from state to state, following the sequence of machine transitions each individual control in the list denotes[fn:25]. (Control lists are also called continuations in this info file.)
...
- OMEGA :: a literal constant used as placeholder for the ~car~ of the environment which is altered by,
...
@@html:-- Function <b>secd-comp--comp</b> expression names controls@@
...
@@html:-- Function <b>rule</b> hypo lhs [rhs context]@@
...
#+CINDEX: backward chaining
#+CINDEX: goal (evaluation)
#+CINDEX: hypothesis (evaluation)
...
#+CINDEX: rule (evaluation)
#+CINDEX: and (evaluation)
In turn, rule evaluation is performed by a sequential evaluation of the conditions in its LHS, following a logical ~AND~. It stops at the first ~*F*~ value for a condition and sets the rule value to ~*F*~; when all the conditions evaluate to ~*T*~, the rule value is set to ~*T*~.

#+CINDEX: rule firing (evaluation)
When a rule is evaluated to ~*T*~ during the sequential process its RHS is immediately executed before proceeding to the next rule in sequence. The rule is said to be /fired/.
...
#+CINDEX: agenda
#+CINDEX: forward-chaining
where ~v~ is a variable name, and ~s~ a well-formed ~funxp~ sexp to compute the value assigned to ~v~. The assignment may trigger forward-chaining consequences, e.g. posting conditions in other rules on the agenda for later evaluations.

Note that evaluation of a RHS sexp and an action sexp individually is always ~nil~. It is ignored by the computation thread.

**** Evaluating conditions on subgoals
#+CINDEX: condition (evaluation)
Conditions are general ~funxp~ boolean sexps. In addition, ~funxp~ offers two boolean forms for conditions:

#+BEGIN_SRC emacs-lisp :results silent :exports code
(H)
(not H)
;; `not' also applies to boolean sexps
(not s)
#+END_SRC

where ~H~ is another hypothesis (and ~s~ any other boolean sexp).

#+CINDEX: backward chaining
#+CINDEX: goal (evaluation)
#+CINDEX: subgoal (evaluation)
When the first two forms are encountered in condition evaluation, the current focus is pushed and the hypothesis ~H~ is evaluated. (This is similar to a procedure call in imperative programming languages, or an application in ~funxp~ as ~H~ is a promise.) In the AI established jargon this is the /chaining/, here in backward mode as it goes from goal to subgoal.
...
#+CINDEX: heuristics
#+FINDEX: secd--kb-option-backward-chaining-rhs
If a variable ~v~ is required, for evaluating a condition, which is not an hypothesis but for which there are some rules in the knowledge base that may set its value in their RHS, a heuristics option in the NXP Architecture exists to trigger backward chaining from ~v~ through the RHS of these rules (rather than asking the user in that instance).

@@html:-- Variable <b>secd--kb-option-backward-chaining-rhs</b>@@
#+ATTR_TEXINFO: :options secd--kb-option-backward-chaining-rhs
#+BEGIN_defvar
When switched to true, search for and evaluate rules with RHS setting data required in condition evaluation before asking the user. Set to ~nil~ by default.
#+END_defvar

**** Suggesting hypotheses in interactive sessions
#+CINDEX: suggest
#+CINDEX: agenda
...
#+CINDEX: forward chaining
(Re)evaluated conditions, in turn, cause (re)evaluation of rules, still with the ~AND~ logical connective described above. (Re)evaluated, may then fire, and in turn cause (re)evaluation of hypotheses. If the hypothesis is itself required in a LHS condition, the forward-chaining proceeds with the new focus of attention. Forward-chaining then proceeds generally from data to subgoals, to goals, in a chain of logical deductions.

**** Gating
#+CINDEX: gating (forward-chaining)
...
@@html:<b>secd--kb-option-forward-chaining-gate</b>@@
#+ATTR_TEXINFO: :options secd--kb-option-forward-chaining-gate
#+BEGIN_defvar
When true, the deductive step or forward chaining from a rule to its hypothesis is taken only if the rule is found ~*T*~ (gating on). When false, the deductive step is always taken whatever the value of the rule (gating off). The default is gating on.
#+END_defvar

*** Heuristics design space
#+CINDEX: heuristics
Backward- and forward-chaining by no means exhaust the possibilities and nuances of control in problem-solving. It is even debatable that computing is all there is to thinking [fn:5], both from the standpoint of the philosophy of mind and from the standpoint of computer science.

Rules may be put to many uses in representing the process of problem-solving, [[Rules are put to many uses in inference]], and together with the various chaining and options already mentioned help explore a large portion of the heuristics design space. In pioneering early AIM systems, other representation schemes were also tested, most notably, /association/ and /weighted association/ (also known as /certainty factors/) relations between data and/or goals, @@texinfo:@xref{Szolovits1982,,33}.@@

#+CINDEX: hypothesis (generation)
#+CINDEX: NCLOSE
The /hypothesis generation/ phase is a well-identified and delimited step in the problem-solving task at large[fn:30]. While rules can be used for hypothesis generation, particularly in forward-chaining mode, AI research studied many alternative mechanisms very early on. (Dr Paycha's address to the 1956 First International Congress of Cybernetics already explained medical hypothesis generation in medical consultation by associational links from signs, to syndromes, to diseases, for instance, @@texinfo:@xref{Paycha1963,,11}.@@) The early NCLOSE system had a separate initial step of hypothesis generation before switching to a "deductive" mode to logically prove or disprove candidate hypotheses, @@texinfo:@xref{Rappaport-1984-15190,,12}.@@


#+CAPTION: Schematic diagram of a system for ill-structured problems. It shows the alternation between a problem solver working on a well-structured problem, and a recognition system continually modifying the problem space. (From Simon, 1973)
#+attr_texinfo: :width 300px :center t
[[./EVOCATION.jpg]]


#+CINDEX: context
#+CINDEX: ill-structured problems
...
#+CINDEX: hypothesis (evocation)
#+CINDEX: transitive closure
...
@@html:-- Variable <b>secd--kb-option-context</b>@@
#+ATTR_TEXINFO: :options secd--kb-option-context
#+BEGIN_defvar
...
@@html:-- Variable <b>secd--kb-option-context-compare</b>@@
#+ATTR_TEXINFO: :options secd--kb-option-context-compare
#+BEGIN_defvar
...
#+CINDEX: NXP Executive
#+CINDEX: knowledge base compiler
...
Implementing the NXP Architecture from the functional perspective provided by ~funxp~ then requires:
  ...
  - embedding the ~funx~ (eager) compiler into a knowledge-base compiler that produces a complete state for the augmented SECD Abstract Machine to process.
...
#+CINDEX: knowcess
@@html:-- Function <b>secd-comp--kb-knowcess</b> env goals [s var val]@@
#+ATTR_TEXINFO: :options secd-comp--kb-knowcess env goals &optional s var val
#+BEGIN_defun
...
@@html:-- Function <b>secd-comp--kb2env</b> kb@@
#+ATTR_TEXINFO: :options secd-comp--kb2env kb
#+BEGIN_defun
...
** Mapping NXP to SECD
#+CINDEX: hypothesis (as a promise)
#+CINDEX: rule (as a promise)
#+CINDEX: condition (as a promise)
#+CINDEX: action (as a promise)
#+CINDEX: variable (as a promise)
...
#+CINDEX: call-by-need
#+CINDEX: suggest (command)
#+CINDEX: volunteer (command)
#+CINDEX: NXP SECD Machine
...
#+CINDEX: production memory (in SECD Abstract Machine)
#+CINDEX: working memory (in SECD Abstract Machine)
#+CINDEX: agenda (in SECD Abstract Machine)
...
#+CINDEX: backward chaining (CPS)
#+CINDEX: forward chaining (CPS)
#+CINDEX: evocation (CPS)
...
#+CINDEX: ASK (answer command)
Execution of the SECD Machine is resumed by issuing the command /answer/ with the optional argument set to ~t~.

@@html:-- Function <b>secd-answer</b> state val [resume]@@
#+ATTR_TEXINFO: :options secd-answer state val &optional resume
#+BEGIN_defun
...
** Backward Chaining
#+CINDEX: backward chaining (implementation)
#+CINDEX: CPS
#+CINDEX: agenda
...
#+CINDEX: ALL (transition)
#+CINDEX: ANY (transition)
#+CINDEX: CPS (transition)
| s             | e | c                       | d                    |
|---------------+---+-------------------------+----------------------|
|               |   |                         |                      |
| (p1 ... pn s) | e | (ALL n . c)             | d                    |
...
#+CINDEX: hypothesis (compiled)
#+CINDEX: LHS (compiled)
#+CINDEX: RHS (compiled)
- Compiled hypothesis :: ~(H PROMISE LDP rN ... LDP r1 ANY N UPD)~, where the ~rI~ are unique ids for the rules with given hypothesis.
- Compiled LHS :: ~(LDP cN ... LDP c1 ALL N)~, where the ~cI~ are unique ids for the conditions in the LHS of the rule.
- Compiled RHS :: ~(LDP aN ... LDP a1 SEQ N)~, where the ~aI~ are unique ids for the RHS actions
- Compiled rule :: ~(rule PROMISE LDP cN ... LDP c1 ALL N SEL (LDP aM ... LDP a1 SEQ M LDC *T* JOIN) (LDC *F* JOIN) UPD)~ when there are ~N~ conditions and ~M~ actions.
...
The knowledge base compiler produces an if-LHS-then-RHS construct for each rule, and a control list that performs a logical ~OR~ on these constructs in order to evaluate their hypotheses. The implementation of the ~ANY~, ~ALL~, and ~SEQ~ operators executes 2 steps: 
  * (i) a pre-execution scan is performed (except for ~SEQ~) looking for promises already forced to ~*T*~ or ~*F*~ according to the logical operator (~OR~, ~AND~) involved in the evaluation. When found, this value is directly propagated upwards to the rule, or to the hypothesis without further computation.
  * (ii) if none is found in the scan, the ~ANY~/~CPS~, ~ALL~/~CPS~, and ~SEQ~/~CPS~ implement a loop through the ~N~ promises on the stack, evaluating each of them in sequence until resulting in ~*T*~ or ~*F*~ according to the logical operator (~OR~, ~AND~) involved in the evaluation; and all of them unconditionally  for ~SEQ~. 
...
** Forward Chaining
#+CINDEX: forward chaining (implementation)
#+CINDEX: agenda
#+CINDEX: gating
#+FINDEX: secd--kb-option-forward-chaining-gate
...
#+CINDEX: hooks
In line with the original design choice of mapping the knowledge base to the environment register of the SECD Machine, the starting event is the assignment of a value to a variable, a condition, or a rule in the environment. We implement forward chaining as a callback to the NXP Executive from the SECD Machine on an update event of the ~e~ register. In the current implementation, the callback mechanism relies on Emacs-Lisp /hooks/, @@texinfo:@xref{Hooks, Hooks, Hooks,elisp, The Emacs Lisp Reference Manual}.@@

The environment register of the ~funxp~ machine supports two hooks:

#+FINDEX: secd-comp--kb-forward-hook
#+FINDEX: secd-comp--kb-knowcess
@@html:-- Variable <b>secd-env-update-hook</b>@@
#+ATTR_TEXINFO: :options secd-env-update-hook
#+BEGIN_defvar
...
by default when calling ~secd-comp--kb-knowcess~, [[Compiling funxp]].

Each hook in the list is passed three arguments: 
- promise :: the variable name (each entity in the knowledge base is represented as a promise in the environment register)
- val :: the value assigned to that promise by the evaluation process
- state :: the current state of the SECD Machine ~(s e c d)~
#+END_defvar

@@html:-- Variable <b>secd-env-locate-hook</b>@@
#+ATTR_TEXINFO: :options secd-env-locate-hook
#+BEGIN_defvar
...
#+CINDEX: CPS
#+CINDEX: heuristics (customization)
#+CINDEX: agenda (control of)
...
** Evocation
#+CINDEX: evocation (implementation)
#+CINDEX: agenda
#+FINDEX: secd-env-update-hook
The evocation also makes use of the ~secd-env-update-hook~. The NXP Executive installs a second default hook for evocation according to the value of the heuristics option: ~secd--kb-option-context~, [[Heuristics design space]].

@@html:-- Function <b>secd-comp--kb-context-hook</b> var val state@@
#+ATTR_TEXINFO: :options secd-comp--kb-context-hook var val state
#+BEGIN_defun
...
#+CINDEX: knowledge base (compiling)
...
#+FINDEX: *BWRD-SIGNS*
- ~*BWRD-SIGNS*~ :: Keeps track of which rules' RHS may set a variable's value. Used in backward chaining when the heuristics option. ~secd--kb-option-backward-chaining-rhs~ is ~t~.
#+FINDEX: *SECD-TOPLEVEL-CLIST*
...
#+FINDEX: *COND-SOURCE*
- ~*COND-SOURCE*~ :: Keeps a copy of the well-formed ~funxp~ LISP variant sexp for each condition. Used in the what-if mechanism to /un-force/ a promise back to its delayed evaluation status.
...
#+FINDEX: *SECD-COMP* (buffer)
#+FINDEX: *SECD-PROTOCOL* (buffer)
#+FINDEX: *NXP-ENCY* (buffer)
#+FINDEX: *NXP-SESSION* (buffer)
#+CINDEX: buffer (trace)
#+CINDEX: interactive session
@@html:-- Function <b>nxp_session</b> kb@@
#+ATTR_TEXINFO: :options nxp-session kb
#+BEGIN_defun
...
- *SECD-COMP* :: Traces the compilation of the knowledge base and report errors.
- *SECD-PROTOCOL* :: Step by step trace of the SECD Machine execution of the top level control list/
- *NXP-SESSION* :: A buffer keeping track of the high-level interactions of Q&A while the session runs.
#+END_defun
...
It displays two tables, one for hypotheses, the other for variables in the knowledge base, with names and values at any given time in the interactive session. It is implemented with the so-called /Ewoc/ library, meaning “Emacs’s Widget for Object Collections”, @@texinfo::@xref{Abstract Display, Ewoc, Ewoc, elisp, The Emacs Lisp Reference Manual}.@@

The Encyclopedia supports a very limited set of commands:
#+CINDEX: Encylopedia (commands)
#+ATTR_TEXINFO: :table-type vtable 
#+BEGIN_QUOTE
  - `q' :: Kill Encyclopedia buffer.
  - `k' :: Suggest hypo at point and knowcess immediately.
  - `w' :: Volunteer, or What-if, data at point and knowcess immediately.
  - `a' :: Answer pending question and resume session.
  - `r' :: Restart session.
  - `t' :: Open backward-chaining tree visualization of hypo at point.
#+END_QUOTE

#+CINDEX: buffer (visualization)
#+FINDEX: *NXP-TREE* (buffer)
...
[fn:30] For illuminating discussions, see Heuristic Methods for Imposing Structure on Ill-Structured Problems: The Structuring of Medical Diagnostics, /Harry E. Pople, Jr./ in Artificial Intelligence in Medicine, /Edited by Peter Szolovits/, AAAS Selected Symposium 51, (1982); The Structure of Ill Structured Problems, /Herbert A. Simon/, Artificial Intelligence 4, p. 181-201, (1973); Clinical Problem Solving: A Behavioral Analysis, /J.P. Kassirer and G.A. Gorry/, Ann. Int. Med. 89, 245, (1978); Medical Problem Solving: An Analysis of Clinical Reasoning, /A.S. Elstein, L.A. Shulman, and S.A. Sprafka/, Harvard University Press, (1978).

[fn:29] See also: Monadic Style Control Constructs for Inference Systems, /Jean-Marie Chauvet/, Computer Science, ArXiv, (2002). 
