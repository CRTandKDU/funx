...
#+CINDEX: EQ
- EQ :: test for equality of popped top two stack values, push back boolean 
result
#+CINDEX: LEQ
- LEQ :: test for lower-than popped top two stack values, push back boolean 
#+CINDEX: SEL
...
| s | e | c                | d | -> | s'                  | e' | c' | d' |
|---+---+------------------+---+----+---------------------+----+----+----|
| s | e | (LDC sexp . c)   | d | -> | (sexp . s)          | e  | c  | d  |
| s | e | (LD x . c )      | d | -> | ((~locate~ x e) . s | e  | c  | d  |
| s | e | (LDF f-sexp . c) | d | -> | ((f-sexp e) . s)    | e  | c  | d  |
...
|----------------+---+-----------------+-------+----+---------------+----+----------+-------|
| ;; Tests       |   |                 |       |    |               |    |          |       |
| (a s)          | e | (ATOM . c)      | d     | -> | ([*T*,*F*] s) | e  | c        | d     |
| (a b s)        | e | (EQ . c)        | d     | -> | ([*T*,*F*] s) | e  | c        | d     |
| (a b s)        | e | (LEQ . c)       | d     | -> | ([*T*,*F*] s) | e  | c        | d     |
|----------------+---+-----------------+-------+----+---------------+----+----------+-------|
| ;; IF control  |   |                 |       |    |               |    |          |       |
| ([*T*,*F*]  s) | e | (SEL cT cF . c) | d     | -> | s             | e  | [cT, cF] | (c d) |
...
|-----------------+----+---------+-----------+----+-------------+---------------------------------+-----------+-----------|
| ;; Application  |    |         |           |    |             |                                 |           |           |
| (f-sexp vals s) | e  | (AP c)  | d         | -> | nil         | ((args-f-sexp . vals) e-f-sexp) | cl-f-sexp | (s e c d) |
| (ret-val)       | e' | (RTN)   | (s e c d) | -> | (ret-val s) | e                               | c         | d         |
|-----------------+----+---------+-----------+----+-------------+---------------------------------+-----------+-----------|
| ;; LET blocks   |    |         |           |    |             |                                 |           |           |
| s               | e  | (DUM c) | d         | -> | s           | (OMEGA e)                       | c         | d         |
| (f-sexp vals s) | (OMEGA e) | (RAP c) | d         | -> | nil         | ~rplaca~((args-f-sexp . vals) e-f-sexp) | cl-f-sexp | (s e c d) | 
...
Nelly Aiello 3 control strategies for rule systems.
